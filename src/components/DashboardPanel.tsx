...
import { 
  getNutritionLogAsync, 
  getWorkoutLogAsync, 
  getWorkoutStreakAsync
} from '@/utils/storage';
...
const DashboardPanel: React.FC<DashboardPanelProps> = ({ userProfile }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setIsLoading(true);
        const [nutrition, workout, workoutStreak] = await Promise.all([
          getNutritionLogAsync(today),
          getWorkoutLogAsync(today),
          getWorkoutStreakAsync()
        ]);
      ...
      <CardContent>
        {isLoading ? (
          <div className="flex justify-center items-center h-64">
            <div className="w-8 h-8 border-4 border-app-teal border-t-transparent rounded-full animate-spin"></div>
          </div>
        ) : (
